...
class LdbFile:
    ...
    def __init__(self, *, path: Path | None = None, fh: BinaryIO | None = None):
        ...
        if not path and not fh:
            raise LevelDBError("LdbFile requires one of path or fh")
        ...
        if self.footer.magic != c_leveldb.LDB_MAGIC:
            raise LevelDBError(f"Invalid LevelDB footer magic {self.footer.magic!r}")
...
class LdbBlock:
    ...
    def entries(self) -> Iterator[tuple[c_leveldb.BlockEntry, c_leveldb.BlockHandle]]:
        ...
        if offset >= self._restart_offset:
            raise LevelDBError("Reading start of entry past the start of restart offset")
        ...
        while self.data.tell() < self._restart_offset:
            ...
            if entry.shared_len > len(key):
                raise LevelDBError("Shared key length is longer than the previous key")
...
class ManifestFile:
    ...
    def __init__(self, *, path: Path | None = None, fh: BinaryIO | None = None):
        ...
        if not path and not fh:
            raise LevelDBError("ManifestFile requires one of path or fh")
